...
void display() {
    ...
    glBegin(GL_LINES);
        ...
        glVertex2f(-0.9f, 0.7f);  // start point(x,y)
        glVertex2f(0.9f, 0.7f);   // ending point(x1, y1)

    glEnd();

    glLineWidth(5.0f);
    // GL_LINE_STRIP automatically connects each new point to the last one
    glBegin(GL_LINE_STRIP);

        glColor3f(1.0f, 0.0f, 0.0f); // Red
        glVertex2f(-0.7f, -0.4f);    // Point 1 (Bottom Left)

        glColor3f(0.0f, 1.0f, 0.0f); // Green
        glVertex2f(-0.3f,  0.4f);    // Point 2 (Top Left-ish)

        glColor3f(0.0f, 0.0f, 1.0f); // Blue
        glVertex2f( 0.3f, -0.4f);    // Point 3 (Bottom Right-ish)

        glColor3f(1.0f, 1.0f, 0.0f); // Yellow
        glVertex2f( 0.7f,  0.4f);    // Point 4 (Top Right)
